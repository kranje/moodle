...
M.quizaccess_lockdownbrowser.timer = {
    ...
    init: function(Y, start, preview) {
        ...
        require(['core_form/changechecker'], function(FormChangeChecker) {
            M.quizaccess_lockdownbrowser.timer.FormChangeChecker = FormChangeChecker;
        });
        Y.one('#toggle-timer').on('click', function() {
            M.quizaccess_lockdownbrowser.timer.toggleVisibility();
        });
    },
    ...
    update: function() {
        ...
        if (secondsleft < 0) {
            ...
            M.quizaccess_lockdownbrowser.timer.FormChangeChecker.markFormSubmitted(input.getDOMNode());
            form.submit();
            return;
        }
        ...
        if (secondsleft < 100) {
            ...
            M.quizaccess_lockdownbrowser.timer.toggleVisibility(true);
        }
    ...
    updateEndTime: function(timeleft) {
        ...
        if (M.quizaccess_lockdownbrowser.timer.Y === null) {
            M.quizaccess_lockdownbrowser.timer.init(window.Y, timeleft, 0);
        }

        // Only update if change is greater than the threshold, so the
        // time doesn't bounce around unnecessarily.
        if (Math.abs(newtimeleft - M.quizaccess_lockdownbrowser.timer.endtime) > M.quizaccess_lockdownbrowser.timer.threshold) {
            M.quizaccess_lockdownbrowser.timer.endtime = newtimeleft;
            M.quizaccess_lockdownbrowser.timer.update();
        }
    }
};

M.quizaccess_lockdownbrowser.filesUpload = {
    /**
     * YUI object.
     */
    Y: null,
    ...
    disableNavPanel: function() {
        ...
        if (quizNavigationBlock) {
            if (M.quizaccess_lockdownbrowser.filesUpload.numberFilesUploading) {
...
M.quizaccess_lockdownbrowser.nav.init = function(Y) {
    ...
    require(['core_form/events'], function(formEvent) {
        document.addEventListener(formEvent.eventTypes.uploadStarted, function() {
            M.quizaccess_lockdownbrowser.filesUpload.numberFilesUploading++;
            M.quizaccess_lockdownbrowser.filesUpload.disableNavPanel();
        });
