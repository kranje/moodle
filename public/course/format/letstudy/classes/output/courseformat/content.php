...
class content extends content_base {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // Build a map of parent section number => delegated section numbers (for subsections).
        $modinfo = get_fast_modinfo($course);
        $subsectionmap = []; // parent section num => [delegated section nums].
        foreach ($modinfo->get_cms() as $cminfo) {
            if ($cminfo->modname === 'subsection') {
                $dsec = $DB->get_record('course_sections', [
                    'course' => $course->id,
                    'component' => 'mod_subsection',
                    'itemid' => $cminfo->instance,
                ]);
                if ($dsec) {
                    $subsectionmap[$cminfo->sectionnum][] = $dsec->section;
                }
            }
        }
        ...
        if ($data->showprogress || $data->islayoutkanban) {
            ...
            if (!empty($data->sections)) {
                foreach ($data->sections as &$sectiondata) {
                    ...
                    // Collect cmids from this section and all its delegated subsections.
                    $allcmids = $modinfo->sections[$sectionnum] ?? [];
                    if (!empty($subsectionmap[$sectionnum])) {
                        foreach ($subsectionmap[$sectionnum] as $delegatednum) {
                            $subcmids = $modinfo->sections[$delegatednum] ?? [];
                            $allcmids = array_merge($allcmids, $subcmids);
                        }
                    }
                    $total = 0;
                    $completed = 0;
                    foreach ($allcmids as $cmid) {
                        ...
                        // Skip subsection modules themselves from completion count.
        ...
        if (!empty($data->sections)) {
            ...
            foreach ($data->sections as &$sectiondata) {
                ...
                // Activity count (include delegated subsection activities).
                $allsecmods = $modinfo->sections[$sectionnum] ?? [];
                if (!empty($subsectionmap[$sectionnum])) {
                    foreach ($subsectionmap[$sectionnum] as $delegatednum) {
                        $subcmids = $modinfo->sections[$delegatednum] ?? [];
                        $allsecmods = array_merge($allsecmods, $subcmids);
                    }
                }
                $visiblecount = 0;
                foreach ($allsecmods as $cmid) {
                    $cm = $modinfo->get_cm($cmid);
